...
def validate_series(
    ...
    *,
    ...
    if x.ndim != 3:
        raise ValueError(f"{name} must have shape [batch, time, feature], got {tuple(x.shape)}")
    ...
    if length is not None and x.shape[1] != length:
        raise ValueError(f"{name} must contain exactly {length} time steps, got {x.shape[1]}")
...
def overlap_average(
    ...
    *,
    ...
    offsets = torch.arange(patch_length, device=patch_scores.device)
    indices = (starts[:, None] + offsets[None, :]).reshape(1, -1).expand(batch, -1)
    values = patch_scores[:, :, None].expand(-1, -1, patch_length).reshape(batch, -1)

    sums = patch_scores.new_zeros(batch, output_length)
    sums.scatter_add_(1, indices, values)
    counts = patch_scores.new_zeros(batch, output_length)
...
def topk_cosine_distance(
    ...
    *,
    ...
    if queries.ndim != 2 or bank.ndim != 2 or queries.shape[1] != bank.shape[1]:
        raise ValueError("queries and bank must be 2-D tensors with equal feature width")
    if bank.shape[0] == 0:
    ...
    normalized_bank = F.normalize(bank, dim=1, eps=1e-12)
    distances: list[Tensor] = []
    for q_start in range(0, queries.shape[0], query_chunk_size):
        query = F.normalize(queries[q_start : q_start + query_chunk_size], dim=1, eps=1e-12)
        best = query.new_full((query.shape[0], k), -torch.inf)
        for b_start in range(0, normalized_bank.shape[0], bank_chunk_size):
            similarities = query @ normalized_bank[b_start : b_start + bank_chunk_size].T
...
def sinusoidal_encoding(length: int, width: int, *, dtype: torch.dtype = torch.float32) -> Tensor:
    ...
    frequencies = torch.exp(
        torch.arange(0, width, 2, dtype=dtype) * (-torch.log(torch.tensor(10_000.0)) / width)
    )
