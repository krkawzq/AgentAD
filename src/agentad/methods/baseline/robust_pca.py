...
def _config(full: np.ndarray, hp: Mapping[str, Any] | None) -> RobustPCAConfig:
...
def evaluate(
    ...
    *,
    ...
    unit = unit_dir(output_root, "baseline/RobustPCA", split)
    results = unit_dir(results_root, "baseline/RobustPCA", split)
    prepare_run(
        unit,
        split,
        method="baseline/RobustPCA",
        partition=partition,
        seed=seed,
        hp=hp,
        resume=resume,
