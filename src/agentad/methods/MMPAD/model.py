...
class MMPAD(nn.Module):
    ...
    def __init__(self, config: MMPADConfig) -> None:
        super().__init__()
        self.config = config
        self._fitted_length: int | None = None
    ...
    def _resolved_length(self, series: Tensor) -> int:
        if self.config.subsequence_length is not None:
            return self.config.subsequence_length
        return _infer_subsequence_length(series)

    def _patches(self, series: Tensor, length: int) -> tuple[Tensor, Tensor]:
    ...
    def fit(self, normal_series: Tensor) -> "MMPAD":
        length = self._resolved_length(normal_series)
        normal_series = validate_series(
            normal_series,
            min_length=length,
            name="normal_series",
        )
        patches, valid = self._patches(normal_series, length)
    ...
    def _window_scores(
        ...
        *,
        ...
        exclusion = int(self.config.exclusion_fraction * length)
        ...
        for start in range(0, query.shape[0], self.config.query_chunk_size):
            ...
            picked = ordered.new_full(
                (ordered.shape[0], slots, self.config.neighbors), -torch.inf
            )
            for slot in range(slots):
                candidates = ordered[:, :, slot].clone()
                for rank in range(self.config.neighbors):
            ...
            for rank in range(1, self.config.neighbors):
    ...
    def _align(
        self, window_scores: Tensor, output_length: int, length: int
    ) -> Tensor:
    ...
    def score(self, series: Tensor) -> Tensor:
        if self.reference_patches.numel():
            ...
            length = self._resolved_length(series)
        series = validate_series(series, min_length=length, name="series")
        with evaluation_mode(self):
            query_patches, query_valid = self._patches(series, length)
            outputs: list[Tensor] = []
            if self.reference_patches.numel():
